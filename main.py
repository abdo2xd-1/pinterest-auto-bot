...
WHATSAPP_PHONE = os.getenv("WHATSAPP_PHONE")  # e.g., 201012345678
PRODUCT_LINK = os.getenv("PRODUCT_LINK", "https://aahmedalmno.gumroad.com/l/gmvtlk")

def get_trending_pin_ideas(count=5):
...
[
  {{
    ...
    "pexels_search": "high quality 2-3 visual keywords (e.g. aesthetic desk setup)",
...
def send_whatsapp_notification(title, pin_url):
    ...
    chat_id = f"{WHATSAPP_PHONE}@c.us" if "@" not in WHATSAPP_PHONE else WHATSAPP_PHONE
    msg = f"🚀 *تم نشر دبوس جديد بنجاح!*\n\n📌 *العنوان:* {title}\n🔗 *الرابط:* {pin_url}\n💰 *رابط المنتج:* {PRODUCT_LINK}"
...
def main():
    ...
    for idx, pin_data in enumerate(pins, 1):
        ...
        if res:
            pin_id = res.get("id", "")
            pin_url = f"[https://www.pinterest.com/pin/](https://www.pinterest.com/pin/){pin_id}/" if pin_id else "Created"
